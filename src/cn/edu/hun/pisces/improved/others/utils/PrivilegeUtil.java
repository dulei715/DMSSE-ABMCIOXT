...
public class PrivilegeUtil {
    ...
    public static Map<cn.edu.hun.pisces.improved.basic.role.Client, List<Keyword>> grantBasePrivilege(Set<Keyword> keywordSet, List<cn.edu.hun.pisces.improved.basic.role.Client> clientList, boolean existAdmin) {
        int keywordNum = keywordSet.size();
        int tenPartKeywordNum = keywordNum / 10;
        Random random = new Random();
        Map<cn.edu.hun.pisces.improved.basic.role.Client, List<Keyword>> map = new HashMap<>();
        for (int i = 0; i < clientList.size(); ++i) {
            cn.edu.hun.pisces.improved.basic.role.Client client = clientList.get(i);
    ...
    public static Map<cn.edu.hun.pisces.improved.advanced.role.Client, List<Keyword>> grantAdvancePrivilege(Set<Keyword> keywordSet, List<cn.edu.hun.pisces.improved.advanced.role.Client> clientList, boolean existAdmin) {
        int keywordNum = keywordSet.size();
        int tenPartKeywordNum = keywordNum / 10;
        Random random = new Random();
        Map<cn.edu.hun.pisces.improved.advanced.role.Client, List<Keyword>> map = new HashMap<>();
        for (int i = 0; i < clientList.size(); ++i) {
            cn.edu.hun.pisces.improved.advanced.role.Client client = clientList.get(i);
            List<Keyword> list = null;
            if (!map.containsKey(client)) {
                list = new ArrayList<>();
                map.put(client, list);
    ...
    public static void writeBasePrivilege(Map<cn.edu.hun.pisces.improved.basic.role.Client, List<Keyword>> clientListMap, String fileName) {
//        String fileName = Constant.DEFAULT_PROGRAM_PATH + "datafile\\improved\\privilege.properties";
        Properties properties = new Properties();

        for (Map.Entry<cn.edu.hun.pisces.improved.basic.role.Client, List<Keyword>> entry : clientListMap.entrySet()) {
            cn.edu.hun.pisces.improved.basic.role.Client client = entry.getKey();
...
//            properties.put(clientPK, stringBuilder.toString());
    ...
    public static void writeAdvancePrivilege(Map<cn.edu.hun.pisces.improved.advanced.role.Client, List<Keyword>> clientListMap, String fileName) {
//        String fileName = Constant.DEFAULT_PROGRAM_PATH +  "datafile\\improved\\privilege_advanced.properties";
        Properties properties = new Properties();

        for (Map.Entry<cn.edu.hun.pisces.improved.advanced.role.Client, List<Keyword>> entry : clientListMap.entrySet()) {
            cn.edu.hun.pisces.improved.advanced.role.Client client = entry.getKey();
            String clientPK = client.getPublicKey().toString();
